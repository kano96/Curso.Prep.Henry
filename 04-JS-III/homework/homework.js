...
function promedioResultadosTest(resultadosTest) {
  ...
  for( var i = 0; i<resultadosTest.length; i++){
    sum = sum + Math.round(resultadosTest[i]);
  }
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  if (multiplicarArgumentos.arguments.length === 0){
    return 0;
  }else if(multiplicarArgumentos.arguments.length === 1){
    return multiplicarArgumentos.arguments[0];
  }else{
    for( var i = 0; i < multiplicarArgumentos.arguments.length; i++){
      producto = producto * multiplicarArgumentos.arguments[i];
    }
    return producto;
  }
}
